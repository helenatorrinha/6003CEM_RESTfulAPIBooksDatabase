/**
 * @module permissions/genres
 * @description Configures authorisation rules using the role-acl library, defining roles and their permissions for the genres model.
 * @requires role-acl
 * @see routes/genres for the route handlers that use these functions
 * @see schemas/genre.json for the JSON Schema definition file
 * @see controllers/validation for the validation functions
 * @see controllers/auth for the authentication middleware
 * @see models/genres for the database functions
 */

const AccessControl = require('role-acl');
const ac = new AccessControl(); // Create a new instance of the AccessControl class

/** User Permissions
 * Grants 'user' role permissions to readNone genres (no permissions).
 * Allows actions: 'readNone'.
 * Denies actions: 'create', 'update', 'delete'.
 */
ac.grant('user').execute('readNone').on('genre'); 

/** Admin Permissions
 * Grants 'admin' role permissions to create, update and delete genres.
 * Allows actions: 'create', 'update', 'delete'.
 * Denies actions: none.
 * In this case, we are excluding the genre_id field from the update permission check.
 */
ac.grant('admin').execute('create').on('genre'); 
ac.grant('admin').execute('update').on('genre', ['*', '!genre_id']); 
ac.grant('admin').execute('delete').on('genre'); 

/** Checks the users permission to create a new genre
 * @function create
 * @param {Object} requester - The user object
 * @returns {object} - The result of the authorisation check
 * @throws {Error} Throws an error if the user does not have permission to create a new genre
 */
exports.create = (requester) =>
    ac.can(requester.role).execute('create').sync().on('genre');

/** Checks the users permission to update an genre
 * @function update
 * @param {Object} requester - The user object
 * @param {number} id - The id of the genre to update
 * @returns {object} - The result of the authorisation check
 * @throws {Error} Throws an error if the user does not have permission to update the genre
 */
exports.update = (requester, id) =>
    ac.can(requester.role).context({requester:requester.user_id, owner:id}).execute('update').sync().on('genre');

/** Checks the users permission to delete an genre
 * @function delete
 * @param {Object} requester - The user object
 * @param {number} id - The id of the genre to delete
 * @returns {object} - The result of the authorisation check
 * @throws {Error} Throws an error if the user does not have permission to delete the genre
 */
exports.delete = (requester, id) => 
  ac.can(requester.role).context({requester:requester.user_id, owner:id}).execute('delete').sync().on('genre');
